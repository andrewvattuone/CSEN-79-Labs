...
namespace lab8_avattuone
{
    ...
    public:
        ...
        void print(); // added this function myself for easy printing in the test file
    ...
    void deque<Item>::operator=(const deque<Item>& source) {
        ...
        for (size_type bp_array_index = 0; bp_array_index < source.bp_array_size; ++bp_array_index)
        {
            ...
            else
            {
                ...
                {
                    // if the front pointer of the source deque is pointing to the current block item, set the front pointer of the current deque to point to the current block item    
                    ...
                    // if the back pointer of the source deque is pointing to the current block item, set the back pointer of the current deque to point to the current block item    
                    ...
                    // set the current value of the deque equal to the current value of the source deque
                    block_pointers[bp_array_index][block_item_index] = source.block_pointers[bp_array_index][block_item_index];
                }
            }
        }
        // update block_pointers_end
        block_pointers_end = block_pointers + (bp_array_size - 1);
    }
    ...
    void deque<Item>::clear () {
        ...
        {
            // if the current block has memory allocated to it, deallocate it and set it to NULL
    ...
    // pushes an entry to the front of the deque
    template <class Item>
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        if (first_bp == NULL)
        {
            ...
            block_pointers[bp_mid - 1] = new value_type[block_size];
            block_pointers[bp_mid - 1][0] = entry; // should it be put at this location?

            front_ptr = back_ptr = block_pointers[bp_mid - 1];
        }
        ...
        {
            // decreases front pointer and sets the value it points to equal to entry
            *(--front_ptr) = entry;
        }
        ...
        {
            // decreases first_bp and allocates memory to it since it needs a new data block
            *(--first_bp) = new value_type[block_size];
            front_ptr = *(first_bp) + block_size - 1;
            // sets front_ptr equal to entry
            *front_ptr = entry;
        }
        ...
        {
            // reserves more memory for the deque
            reserve();
            // decreases first_bp and allocates memory to it since it needs a new data block
            *(--first_bp) = new value_type[block_size];
            front_ptr = *(first_bp) + block_size - 1;
    ...
    void deque<Item>::push_back(const value_type& entry)
    {
        ...
        if (last_bp == NULL)
        {
            ...
            block_pointers[bp_mid - 1] = new value_type[block_size];
            block_pointers[bp_mid - 1][0] = entry;
        ...
        {
            // increases last_bp and allocates memory to it since it needs a new data block
            *(++last_bp) = new value_type[block_size];
            back_ptr = *last_bp;
            // sets back_ptr equal to entry
            *back_ptr = entry;
        }
    ...
    // pops the front entry of the deque
    template <class Item>
    void deque<Item>::pop_front()
    {
        ...
        {
            // deallocate memory from the current data block and set it to NULL
            delete[] (*(first_bp));
            *first_bp = NULL;
            // increase first_bp and set front_ptr to point to the first element of the new front data block
            front_ptr = block_pointers[++first_bp - block_pointers];
        }
    ...
    // pops the last entry of the deque
    template <class Item>
    void deque<Item>::pop_back()
    {
        ...
        // This is the last entry of the data block; move to the previous block
        else if (back_ptr == *last_bp)
        {
            // deallocate memory from the current data block and set it to NULL
            delete[] (*(last_bp));
            *last_bp = NULL;
            // increase last_bp and set back_ptr to point to the last element of the new back data block
    ...
    // returns the back entry of the deque
    template <class Item>
    ...
    // returns the front entry of the deque
    template <class Item>
    ...
    // prints each element of the deque
    template <class Item>
    void deque<Item>::print()
    {
        for(deque<Item>::iterator it = begin(); it != end(); it++)
        {
