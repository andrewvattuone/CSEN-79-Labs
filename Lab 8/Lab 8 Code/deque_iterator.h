...
namespace lab8_avattuone
{
    ...
    public:
        ...
        // overloaded != operator
        // returns true if the iterators are not equal (at least one of the private variables for both of the iterators have different values), false otherwise
        ...
        // overloaded == operator
        // returns true if the iterators are equal (every single one of their private variables will be equal in this case), false otherwise
        bool operator ==(const deque_iterator other) const
        {
            return (block_pointers == other.block_pointers && block_pointers_end == other.block_pointers_end
                  && first_bp == other.first_bp  && last_bp == other.last_bp
        ...
        deque_iterator& operator ++( ) // Prefix ++
        {
            ...
            {
                // increase current_block_pointer and set cursor to point to the first element of the next data block
                ++current_block_pointer;
                cursor = *current_block_pointer;
                // set current_boundary to point to the last element of the next data block
