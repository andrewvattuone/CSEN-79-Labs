...
namespace lab9_avattuone
{
    // Traverses the tree using inorder traversal and applies the inputted function to each node
    template <class Process, class BTNode>
    void inorder(Process f, BTNode* node_ptr)
    // Library facilities used: cstdlib
    {
        // As long as the root pointer isn't null, it keeps on performing inorder travesal through the tree
        if (node_ptr != NULL)
        {
    ...
    // Traverses the tree using postorder traversal and applies the inputted function to each node
    template <class Process, class BTNode>
    void postorder(Process f, BTNode* node_ptr)
    // Library facilities used: cstdlib
    {
        // As long as the root pointer isn't null, it keeps on performing postorder travesal through the tree
        if (node_ptr != NULL)
        {
    ...
    // Traverses the tree using preorder traversal and applies the inputted function to each node
    template <class Process, class BTNode>
    void preorder(Process f, BTNode* node_ptr)
    // Library facilities used: cstdlib
    {
        // As long as the root pointer isn't null, it keeps on performing preorder travesal through the tree
        if (node_ptr != NULL)
        {
            f(node_ptr);
    ...
    // Prints out every element in the tree in right root left order
    template <class Item, class SizeType>
    void print(binary_tree_node<Item>* node_ptr, SizeType depth)
    ...
    // Deletes every element from the tree using postorder
    template <class Item>
    void tree_clear(binary_tree_node<Item>*& root_ptr)
    // Library facilities used: cstdlib
    {
        binary_tree_node<Item>* child;
        // If the root pointer is null, it keeps traversing the tree with postorder and deletes the nodes
        if (root_ptr != NULL)
        {
            child = root_ptr->left( );
            tree_clear( child );
    ...
    // Makes a copy of the inputted tree and returns a pointer to the root of the new tree
    template <class Item>
    binary_tree_node<Item>* tree_copy(const binary_tree_node<Item>* root_ptr)
    // Library facilities used: cstdlib
    {
        ...
        // returns NULL if the current root pointer is NULL
        if (root_ptr == NULL)
            return NULL;
        // each node is copied and returned in left right root order
        else
        {
    ...
    // Library facilities used: cstdlib
    {
        // If a leaf node is reached or the tree is empty, stop counting in that direction and return 0
        if (node_ptr == NULL)
            return 0;
        // add 1 since a non-NULL node was found, and keep looking for more nodes, first in the left subtree of the current node and then in its right subtree
        else
